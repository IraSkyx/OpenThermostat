...
void WifiManager::initAP()
{
	...
	Application::getInstance().AddLog(F("Wifi AP mode"));
}
...
void WifiManager::initWifi()
{
	...
	if (Application::getInstance().Wssid == "ESP8266")
	{
	...
	else // Sinon le fichier de sauvegarde n'a pas de donn̩es de connexion sauvegard̩es
	{
		...
		char* ssid = new char[Application::getInstance().Wssid.length() + 1];
		strcpy(ssid, Application::getInstance().Wssid.c_str());
		// Conversion String en char*
		char* pwd = new char[Application::getInstance().Wpass.length() + 1];
		strcpy(pwd, Application::getInstance().Wpass.c_str());
		WiFi.mode(WIFI_STA);
		...
		if (connexion(ssid, pwd, 20)) // 20 = 20 secondes avant l'estimation de l'echec
		{
			...
			Application::getInstance().AddLog("Connected to network");
		}
...
bool WifiManager::connexion(char *ssid, char *pwd, int expiration)
{
	...
	DEBUG_PRINTLN(Application::getInstance().Ip);

	//Application::getInstance().AddLog("IP: "+Application::getInstance().Ip);
	if (Application::getInstance().Ip != "0.0.0.0" && Application::getInstance().Ip != "")
	{
		byte ip[4];
		parseBytes(Application::getInstance().Ip.c_str(), '.', ip, 4, 10);
		byte gat[4];
		parseBytes(Application::getInstance().Gat.c_str(), '.', gat, 4, 10);
		byte dns[4];
		parseBytes(Application::getInstance().Dns.c_str(), '.', dns, 4, 10);
	...
	if (timeout < expiration) // Si la connexion a reussi
	{
		...
		Application::getInstance().AddLog("IP address: " + WiFi.localIP());
		aRetourner = true;
	}
...
String WifiManager::getWiFiInfo()
{
	...
	String aRetourner = WiFi.SSID() + ";" + adresseIP + ";" + Application::getInstance().Wssid + ";" + Application::getInstance().Wpass + ";";
	// String aRetourner = Application::getInstance().Wssid + ";" + Application::getInstance().Wpass + ";";
