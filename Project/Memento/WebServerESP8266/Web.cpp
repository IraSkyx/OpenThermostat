...
void Web::init()
{
	...
	httpUpdater.setup(&server, "/up_firmware", "admin", Application::getInstance().pwd.c_str());
	//route declaration
...
void Web::handle_root()
{
	...
	if (userVal != Application::getInstance().pwd) // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		if (Application::getInstance().Access == false)
		{
			String r = "";
	...
	{
		Application::getInstance().Access = true;
	}
	String str = "";
	root(lsensor, ldomo, str);
...
void Web::modee()
{
	...
	else
	{
		...
		Application::getInstance().Mode = mode;

		//get temperature
		if (mode == F("Forced"))
		{
			Application::getInstance().Temp = ftemp.toFloat();
			Application::getInstance().FTemp = ftemp.toFloat();
		}
		else
		{
			Application::getInstance().FTemp = 0.0;
			switch (mode.charAt(0))
			{
			case 'E':
				Application::getInstance().Temp = lthermos->Eco;
				break;
			case 'F':
				Application::getInstance().Temp = lthermos->Frostfree;
				break;
			case 'C':
				Application::getInstance().Temp = lthermos->Comfort;
				break;
			default:
				Application::getInstance().Temp = 0.0;
			}
		}
		Application::getInstance().save();
		DEBUG_PRINTLN(F("new config"));
		Application::getInstance().AddLog(F("new config"));
	}
	if (mode == "Off")
	{
		lthermos->off();
...
void Web::wifiCredentials()
{
	...
	if (nssid != "") // Si l'utilisateur saisit des valeurs dans l'URL on modifie la connexion Wifi
	{
		...
		{
			Application::getInstance().Wssid = nssid;
			Application::getInstance().Wpass = npwd;
			Application::getInstance().save();
			server.stop();
...
void Web::calibrateT()
{
	...
	{
		Application::getInstance().TempE = userVal.toFloat();
		Application::getInstance().save();
		server.send(200, "text/plain", "Done");
	}
	else // Sinon on lui affiche la valeur prÃ©sente dans la mÃ©moire
	{
		server.send(200, "text/plain", String(Application::getInstance().TempE));
	}
}
...
void Web::calibrateH()
{
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().HumE = userVal.toFloat();
		Application::getInstance().save();
		server.send(200, "text/plain", "Done");
	}
	else // Sinon on lui affiche la valeur prÃ©sente dans la mÃ©moire
	{
		...
		server.send(200, "text/plain", String(Application::getInstance().HumE));
	}
}
...
void Web::saveconfig()
{
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().pwd = userVal;
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().Wssid = userVal;
	}
	else
		b = true;
	userVal = server.arg("pwd");
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().Wpass = userVal;
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().Ip = userVal;
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().Gat = userVal;
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().Dns = userVal;
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().Ntp = userVal;
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().Timezone = userVal.toInt();
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().dhtPin = userVal.toInt();
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().dhtType = userVal.toInt();
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().pirPin = userVal.toInt();
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().irPin = userVal.toInt();
	}
	else
		b = true;
	...
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
	{
		Application::getInstance().AP = userVal;
	}
	else
		b = true;
	//thermostat
...
void Web::saveplanning()
{
	...
	if (!b && userVal != "")
	{
		...
		if (userVal == "1")
			lthermos->save_Cal(cmd + F("2"));
		userVal = server.arg("ChkTue");
		if (userVal == "1")
			lthermos->save_Cal(cmd + F("3"));
		userVal = server.arg("ChkWed");
		if (userVal == "1")
			lthermos->save_Cal(cmd + F("4"));
		userVal = server.arg("ChkThu");
		if (userVal == "1")
			lthermos->save_Cal(cmd + F("5"));
		userVal = server.arg("ChkFri");
		if (userVal == "1")
			lthermos->save_Cal(cmd + F("6"));
		userVal = server.arg("ChkSat");
		if (userVal == "1")
			lthermos->save_Cal(cmd + F("7"));
		userVal = server.arg("ChkSun");
		if (userVal == "1")
			lthermos->save_Cal(cmd + F("1"));
	}
	DEBUG_PRINTLN(cmd);
	if (!b)
	{
...
void Web::savedomo()
{
	...
	if (userVal == "1") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
		Application::getInstance().domo = true;
	else
		Application::getInstance().domo = false;
	userVal = server.arg("domoIP");
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
		Application::getInstance().domoIP = userVal;
	else
		b = true;
	userVal = server.arg("domoPir");
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
		Application::getInstance().domoPir = userVal.toInt();
	else
		b = true;
	userVal = server.arg("domoTemp");
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
		Application::getInstance().domoTemp = userVal.toInt();
	else
		b = true;
	userVal = server.arg("domogTemp");
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
		Application::getInstance().domogTemp = userVal.toInt();
	else
		b = true;
	userVal = server.arg("domoMode");
	if (userVal != "") // Si l'utilisateur Ã  saisit une valeur alors on Ã©crit dans la mÃ©moire cette valeur
		Application::getInstance().domoMode = userVal.toInt();
	else
		b = true;
...
bool Web::hasAccess(String & str)
{
	if (Application::getInstance().Access == false && Application::getInstance().pwd != "")
	{
...
void Web::sensor(SensorManager *sen, String &str)
{
	if (!hasAccess(str))
		return;
...
void Web::root( SensorManager *sen, DomoticzBroadcaster *ldomo, String &str)
{
	if (!hasAccess(str))
		return;
	head(str);
	//Add_refresh(str);
	if (Application::getInstance().pwd == "")
		str += F("<h3>No password set!</h3>");
	str += F("<h3>Wifi Info</h3>");
	str += "<p>Wifi SSID: " + Application::getInstance().Wssid + " </p></br>";
	Add_form(str, F("mode"));
	Add_table(str, F("Thermostat"));
	str += F("<td><label for=\"mode\">Mode</label></td><td><select name=\"mode\" id=\"mode\">");
	String opt1 = "";
	mode(opt1, Application::getInstance().Mode);
	str += opt1;
	str += F("</select></td></tr>");
	str += F("<tr><td><label>Current Temperature</label></td><td><label>");
	if (Application::getInstance().domo && Application::getInstance().domogTemp != 0)
		str += String(ldomo->getTemp());
	else
		str += String(sen->getTemp());
	str += F("</label></td></tr><tr>");
	Add_input(str, F("expectedT"), F("Expected Temperature"), String(Application::getInstance().Temp));
	str += F("</tr>");
	Add_input(str, F("forcedT"), F("Forced Temperature"), String(Application::getInstance().FTemp));
...
void Web::formconfig(Thermostat *t, String &str, String &error)
{
	if (!hasAccess(str))
		return;
	head(str);
	...
	Add_input(str, F("spwd"), F("Password"), Application::getInstance().pwd);
	str += F("</tr></tbody></table>");
	Add_table(str, F("Network configuration"));
	Add_input(str, F("ssid"), F("Ssid"), Application::getInstance().Wssid);
	str += F("</tr><tr>");
	Add_input(str, F("pwd"), F("Password"), Application::getInstance().Wpass);
	str += F("</tr><tr>");
	Add_input(str, F("ip"), F("IP Adress"), Application::getInstance().Ip);
	str += F("</tr><tr>");
	Add_input(str, F("gat"), F("IP Gateway"), Application::getInstance().Gat);
	str += F("</tr><tr>");
	Add_input(str, F("dns"), F("IP DNS"), Application::getInstance().Dns);
	str += F("</tr><tr>");
	Add_input(str, F("ntp"), F("NTP server"), Application::getInstance().Ntp);
	str += F("</tr><tr>");
	Add_input(str, F("tzone"), F("Time Zone"), String(Application::getInstance().Timezone));
	str += F("</tr><tr>");
	str += F("</tbody></table>");
	Add_table(str, F("Sensor configuration"));
	str += F("<td><label for=\"dhtpin\">DHT Pin</label></td><td><select name=\"dhtpin\" id=\"dhtpin\">");
	String opt1 = "";
	gpio(opt1, Application::getInstance().dhtPin);
	str += opt1;
	str += F("</select></td></tr>");
	...
	String opt2 = "";
	gpio(opt2, Application::getInstance().pirPin);
	str += opt2;
	str += F("</select></td></tr>");
	str += F("<tr><td><label for=\"irpin\">IR Pin</label></td><td><select name=\"irpin\" id=\"irpin\">");
	String opt3 = "";
	gpio(opt3, Application::getInstance().irPin);
	str += opt3;
	str += F("</select></td></tr>");
	Add_input(str, F("HumE"), F("Calibrate Humidity"), String(Application::getInstance().HumE));
	str += F("</tr><tr>");
	Add_input(str, F("TempE"), F("Calibrate Temperature"), String(Application::getInstance().TempE));
	str += F("</tr><tr>");
	str += F("</tbody></table>");
	Add_table(str, F("HeatPump configuration"));
	str += F("<td><label for=\"model\">HeatPump model</label></td><td><select name=\"model\" id=\"model\">");
	String opt4 = "";
	model(opt4, Application::getInstance().AP);
	str += opt4;
...
//log form
void Web::formlog(String &str)
{
	if (!hasAccess(str))
		return;

	uint8_t i = 0;
	uint8_t nlogs = Application::getInstance().getNblogs();

	head(str);
	Add_refresh(str);
	Add_table(str, F("Logs"));
	if (Application::getInstance().getNblogs() == 0)
		str += F("No logs</td></tr>");
	else
	{
		i = Application::getInstance().getFirstlog();
		do
		{
			str += F("<tr><td>");
			str += Application::getInstance().getLogs()[i];
			str += F("</td></tr>");
			i = (i + 1) % NBLOGS;
			nlogs--;
		} while (nlogs>0);
...
//Domoticz form
void Web::formdomo(String &str, String &error)
{
	if (!hasAccess(str))
		return;
	head(str);
	...
	if (Application::getInstance().domo == true)
		str += F(" checked ");
	str += F("\"/></td></tr>");
	Add_input(str, F("domoIP"), F("Domoticz server IP"), String(Application::getInstance().domoIP));
	str += F("</tr><tr>");
	Add_input(str, F("domoPir"), F("Domoticz PIR idx"), String(Application::getInstance().domoPir));
	str += F("</tr><tr>");
	Add_input(str, F("domoTemp"), F("Domoticz Temp idx"), String(Application::getInstance().domoTemp));
	str += F("</tr><tr>");
	Add_input(str, F("domogTemp"), F("Domoticz get Temp idx"), String(Application::getInstance().domogTemp));
	str += F("</tr><tr>");
	Add_input(str, F("domoMode"), F("Domoticz selector idx (Selector with labels Off, Auto, Frostfree, Eco, Comfort, Forced)"), String(Application::getInstance().domoMode));
	str += F("</tr></tbody></table>");
	Add_button(str, F("btn-b"), F("Submit"));
	str += F("</form></div>");
	end(str);
