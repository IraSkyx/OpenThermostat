...
Application config("f5.txt", &sessionManager);
TimeNtp timentp;
...
void runthseconds()
{
  //loggingloop++;
    //if (loggingloop==10)
      //config.Access=false;
  config.sessionManager->flagInvalidSessions();
  cleanupProcess.clean(config.sessionManager->getSession());
   
   //update next timer
  timerth = millis() + 30000;
  config.sessionManager->getSession()->setTimePass(timerth);
