...
DomoticzBroadcaster::DomoticzBroadcaster(){}
...
url+= String(Application::getInstance().domogTemp);
DEBUG_PRINTLN(F("get Temp from Domoticz"));
http.begin(Application::getInstance().domoIP,8080,url);
int httpCode = http.GET();
if (httpCode != 200) {
  //error
  DEBUG_PRINTLN(F("Error Calling Domoticz Temp"));
  Application::getInstance().AddLog(F("Error Calling Domoticz Temp"));
http.end();
return -1;}
else {
...
url+= String(Application::getInstance().domoPir);
url+= F("&nvalue=");
url+= String(state);
DEBUG_PRINTLN(F("send Pir to Domoticz"));
if (state==1)
Application::getInstance().AddLog(F("send Pir 1 to Domoticz"));
else Application::getInstance().AddLog(F("send Pir 0 to Domoticz"));
...
if (httpCode != 200) {
  ...
  Application::getInstance().AddLog(F("Error Calling Domoticz Pir"));
  }
  http.end();
}
...
HTTPClient http;
String url= F("/json.htm?type=command&param=udevice&idx=");
url+= String(Application::getInstance().domoTemp);
url+=F("&nvalue=0&svalue=");
url+= String(temp,2);
DEBUG_PRINTLN(F("send Temp to Domoticz"));
Application::getInstance().AddLog(F("send Temp to Domoticz"));
http.begin(Application::getInstance().domoIP,8080,url);
int httpCode = http.GET();
if (httpCode != 200) {
  //error
...
if (Application::getInstance().Mode!="Auto" && Application::getInstance().Mode!=mode) return;
...
Application::getInstance().AddLog(F("send Mode to Domoticz"));

HTTPClient http;
String url= F("/json.htm?type=command&param=switchlight&idx=");
url+= String(Application::getInstance().domoMode);
...
Application::getInstance().AddLog(F("Error Calling Domoticz Mode"));
}
http.end();
}
