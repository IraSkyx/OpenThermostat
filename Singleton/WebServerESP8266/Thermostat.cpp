...
Thermostat::Thermostat(String cal, String s, SensorManager *se, IRManager *ir,DomoticzBroadcaster *domo)
{
  lsensor=se;
  lIR=ir;
  file=s;
...
void Thermostat::init()
{
  ...
  if (Cal != NULL)
  {
    ...
    for(i=0;i<Calidx;i++)
      {
        Alarm.disable(AlarmTab[i]);
        Alarm.free(AlarmTab[i]);
      }
    delete AlarmTab;
  }
  ...
  for (i=0;i<Calidx;i++)
  {
    ...
    if (firstValue == "1")
    {
      ...
      switch(fourthValue.charAt(0))
      {
        ...
        case '8':
          for(j=2;j<=6;j++)
            setAlarm(nc++,thirdValue.charAt(0),j,h.toInt(),sec.toInt());
          break;
  ...
  if(Application::getInstance().Mode=="Auto")
  {
    Autochecking();
    // if (Application::getInstance().domo) ldomo->sendMode(modeCal);
  }
  if(Application::getInstance().Mode != "Off")
  {
    on();
  }
  else if (Application::getInstance().domo)
  ...
  switch(mode)
  {
    case 'O':
      if (nbday==0)
        ...
        AlarmTab[i]=Alarm.alarmRepeat((timeDayOfWeek_t)nbday,h,s,0, globalOff);
      break;
    ...
    case 'E':
      if (nbday==0)
        ...
        AlarmTab[i]=Alarm.alarmRepeat((timeDayOfWeek_t)nbday,h,s,0, globalEco);
      break;
    ...
    case 'C':
      if (nbday==0)
        ...
        AlarmTab[i]=Alarm.alarmRepeat((timeDayOfWeek_t)nbday,h,s,0, globalComfort);
      break;
    ...
    case 'F':
      if (nbday==0)
        ...
        AlarmTab[i]=Alarm.alarmRepeat((timeDayOfWeek_t)nbday,h,s,0, globalFrostfree);
      break;
...
void Thermostat::run()
{
  ...
  if (Application::getInstance().Mode=="Auto")
    ...
    currentMode=Application::getInstance().Mode;

  //get temperature
  if (Application::getInstance().Mode=="Forced")
    expectedTemp=Application::getInstance().FTemp;
  else
  {
  ...
  Application::getInstance().Temp=expectedTemp;
  ...
  Application::getInstance().AddLog("---Thermostat Mode: "+currentMode+ " Current Temp:" + String(currentTemp)+ " Exp Temp:" + String(expectedTemp)+ " HeatpumpState :" + String(HPState)+"---");
  ...
  if (presentbool==true)
  {
    ...
    if (hour()>=presmin && hour()<=presmax && difference <=prestime*60 && lsensor->getPircount()>2 && currentMode=="Eco")
    {
      ...
      expectedTemp=Comfort;
      Application::getInstance().AddLog("First detection since "+ String(difference2/60));
      Application::getInstance().AddLog("Thermostat: mode eco but presence detected since " +String(difference/60) +  " min-> Comfort mode");
    }
  }
  ...
  if (currentMode != "Off" && HPState==0 && (currentTemp <= expectedTemp - hysteresis))
  {
    ...
    Application::getInstance().AddLog(F("Thermostat: Heatpump start"));
    result=lIR->sendIR(Application::getInstance().AP,"1","2","0",String(expectedTemp),"0","0");
    if (result==0)
    {
  ...
  if (currentMode == "Off" && HPState==1)
  {
    ...
    Application::getInstance().AddLog(F("Thermostat: Heatpump stop"));
    result=lIR->sendIR(Application::getInstance().AP,"0","0","0","0","0","0");
    if (result==0)
    {
      DEBUG_PRINTLN(F("IR error"));
    }
  ...
  if (currentTemp >= expectedTemp + triggerHeat)
  {
    if (HPState==1)
    {
      ...
      Application::getInstance().AddLog(F("Thermostat:  temp difference important, Heatpump shutdown"));
      result=lIR->sendIR(Application::getInstance().AP,"0","0","0","0","0","0");
      if (result==0)
      {
        DEBUG_PRINTLN(F("IR error"));
  ...
  if (HPState == 1 && (HPTemp > expectedTemp+hysteresis))
  {
    ...
    Application::getInstance().AddLog(F("Thermostat: reduce temp Heat pump"));
    result=lIR->sendIR(Application::getInstance().AP,"1","2","0",String(expectedTemp),"0","0");
    if (result==0)
    {
      DEBUG_PRINTLN(F("IR error"));
  ...
  if (HPState == 1 && (HPTemp  < expectedTemp-hysteresis))
  {
    ...
    Application::getInstance().AddLog(F("Thermostat: increase temp Heat pump"));
...
void Thermostat::off()
{
  ...
  Application::getInstance().AddLog(F("Thermostat Off"));
  unsigned int HPState= lIR->getHPState();
  checkConfort = 0;
  // Eteindre
  if (HPState == 1)
  {
    lIR->sendIR(Application::getInstance().AP, "0", "0", "0","0", "0", "0");
  }
  if(Application::getInstance().domo)
    ldomo->sendMode(F("Off"));
}
...
void Thermostat::on()
{
  ...
  Application::getInstance().AddLog(F("Thermostat On"));
  if(Application::getInstance().domo)
  {
    ldomo->sendMode(Application::getInstance().Mode);
    if (Application::getInstance().Mode=="Auto")
      ldomo->sendMode(modeCal);
  }
  //run();
...
void Thermostat::Autochecking(void)
{
  ...
  if (timeStatus()==timeSet && Application::getInstance().Mode=="Auto")
  {
    for (i=0;i<Calidx;i++)
    {
